...
namespace SinglaMedicos.ViewModel
{
    public class ItemsDatabase
    {
        ...
        public void CustomerMasterDB()
        {
            if (AppSettings.CustomersyncStatus == false)
            {
                ...
                try
                {
                    ...
                    string content = _client.GetStringAsync(Baseurl).Result;
                    AppSettings.CustomerListjson = content;
                    customers = JsonConvert.DeserializeObject<List<CustomerMaster>>(content);
        ...
        //            try
        //            {
        //                var localCustorderdetail = Task.Run(async () => await database.Table<Orders>().Where(i => i.order_id != null && i.AzureStatus == "Complete.").ToListAsync()).Result;

        //                foreach (var orddata in localCustorderdetail)
        //                {
        //                    var Custorderdetail1 = Task.Run(async () => await App.ServiceClient.GetTable<SinglaMedicos.Azure.Orders>().ToListAsync()).Result;
        //                    var Custorderdetail = Task.Run(async () => await App.ServiceClient.GetTable<SinglaMedicos.Azure.Orders>().Where(i => i.OrderGuid == orddata.OrderGuid).ToListAsync()).Result.FirstOrDefault();
        //                    if (Custorderdetail != null)
        //                    {
        //                        string qry = "update Orders set order_status='" + Custorderdetail.order_status + "', Invoice_date='" + Custorderdetail.Invoice_date + "',Invoice_No='" + Custorderdetail.Invoice_No + "',UpdateStatus='" + Custorderdetail.UpdateStatus + "' where  order_id='" + Custorderdetail.order_id + "' ";
        //                        var DATA = Task.Run(async () => await database.QueryAsync<Orders>(qry)).Result;
        //                    }
        //                }
        //            }
        //            catch (Exception ex)
        //            {
        //                Analytics.TrackEvent("ItemsDatabase--UpdateOrderIdandStatus--Error : " + AppSettings.salesmancode + "--" + ex.Message);
        //            }
        //        }
        ...
        public void GetCustomerPendingBill(int patnercode, string custcode)
        {
            ...
            try
            {
                ...
                string Baseurl = ReportsUrl + "ReportManagement/GetOutstandingPendingBillsbyCustomer?Partnercode=" + patnercode + "&Custcode=" + custcode + "";
                string content = _client.GetStringAsync(Baseurl).Result;
        ...
        public async void AppcustdeliveryInsertdata()
        {
            try
            {
                ...
                //        SinglaMedicos.Azure.AppCustDlvry custdelvry = new SinglaMedicos.Azure.AppCustDlvry();
                //        var CustDlvrydetail = Task.Run(async () => await App.ServiceClient.GetTable<SinglaMedicos.Azure.AppCustDlvry>().Where(i => i.DlvryCode == appcustdelvry.DlvryCode).ToListAsync()).Result;
                ...
                //            try
                //            {
                //                await App.ServiceClient.GetTable<SinglaMedicos.Azure.AppCustDlvry>().InsertAsync(custdelvry);
                //            }
                //            catch (Exception ex)
                //            {
                //            }
        ...
        public async void AppcustInvoiceupdatedata()
        {
            ...
            //    {
            //        var CustinvoiceDetail = Task.Run(async () => await App.ServiceClient.GetTable<SinglaMedicos.Azure.AppcustInvoice>().Where(i => i.DlvryStatus == "y" && i.invnum == custinvoice.invnum).ToListAsync()).Result;
            //        if (CustinvoiceDetail.Count == 0)
            //        {
            //            SinglaMedicos.Azure.AppcustInvoice appcustdelvry = new SinglaMedicos.Azure.AppcustInvoice();
            ...
            //            try
            //            {
            //                await App.ServiceClient.GetTable<SinglaMedicos.Azure.AppcustInvoice>().UpdateAsync(appcustdelvry);
            //            }
            //            catch (Exception ex)
            //            {
            //            }
