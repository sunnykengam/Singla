...
public partial class InvoiceDetails : ContentPage
    {
        ...
        public InvoiceDetails()
        {
            ...
            //var invoicedetails = Task.Run(async () => await App.ServiceClient.GetTable<SinglaMedicos.Azure.AppcustInvoice>().Where(j => j.custcode == AppSettings.Customercode && j.DlvryStatus == null).OrderByDescending(k => k.invdate).ToListAsync()).Result.ToList();
            ...
            //    try
            //    {
            //        foreach (SinglaMedicos.Azure.AppcustInvoice item in invoicedetails)
            //        {
        ...
        private void insertinvoiceinfo(AppcustInvoice Insertdata, ref string invoicedetail, ref string invoicedate, ref string custname, ref string invodate)
        {
            try
            {
                foreach (object data in chkboxes)
                {
                    ...
                    //var CustinvoiceDetail = Task.Run(async () => await App.ServiceClient.GetTable<SinglaMedicos.Azure.AppcustInvoice>().Where(i => i.invnum == InvocieNo).ToListAsync()).Result;
                    ...
                    //    try
                    //    {
                    //        //App.ServiceClient.GetTable<SinglaMedicos.Azure.AppcustInvoice>().UpdateAsync(CustinvoiceDetail[0]);
                    //    }
                    //    catch (Exception ex)
                    //    {
                    //    }
        ...
        private async void Insertdelvryinfo(AppcustInvoice Insertdata, string invoicedate, string custname, string invoicedetail,byte[] bitmapData)
        {
            try
            {
                //SinglaMedicos.Azure.AppCustDlvry InsertFields = new SinglaMedicos.Azure.AppCustDlvry();
                ...
                //await App.ServiceClient.GetTable<SinglaMedicos.Azure.AppCustDlvry>().InsertAsync(InsertFields);
            }
            catch (Exception ex)
            {
            }
