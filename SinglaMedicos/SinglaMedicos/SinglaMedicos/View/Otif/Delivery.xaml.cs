            ...
            if (CrossConnectivity.Current.IsConnected)
            {
                ...
                if (!string.IsNullOrEmpty(username.Text))
                {
                    try
                    {
                        ...
                        //customerdetail = Task.Run(async () => await App.ServiceClient.GetTable<SinglaMedicos.Model.CustomerMaster>().Where(i => i.customercode == AppSettings.Customercode).ToListAsync()).Result.ToList().FirstOrDefault();
